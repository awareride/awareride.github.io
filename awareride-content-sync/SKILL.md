...
files that exist only on the hub are left untouched. To remove a page from
the hub, delete it here and also remove it on the hub side (the sync does not
delete hub-only files, which protects content contributed by other projects).
